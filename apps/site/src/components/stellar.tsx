/*
 * The official Stellar mark and lockup, from stellar.org's own asset.
 *
 * Used only to say which network this runs on. Both inherit `currentColor` so
 * they sit in the page's ink rather than importing a second brand palette.
 */

/** Full path data of the official lockup: mark at x 0–236, wordmark after it. */
const LOCKUP =
...
export function StellarMark({ className = "" }: { className?: string }) {
  return (
    <svg viewBox="0 0 240 200" className={className} aria-hidden fill="currentColor">
      <path d={LOCKUP} />
    </svg>
  );
}
...
export function StellarLockup({ className = "" }: { className?: string }) {
  return (
    <svg
...
/** "Built on Stellar", for the footer. */
export function BuiltOnStellar() {
  return (
    ...
    >
      ...
      <StellarLockup className="h-[0.95rem] w-auto" />
    </a>
  );
}
